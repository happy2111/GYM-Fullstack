...
import MembershipSkeletonExample
  from "./Skeletons/MembershipSkeletonProfileInfo.jsx";
import MembershipSkeletonProfileInfo
  from "./Skeletons/MembershipSkeletonProfileInfo.jsx";
...
const ProfileInfo = observer(({isProfileRoot}) => {
  ...
  return (
    <div className={"flex max-md:flex-col justify-between gap-6 w-full"}>
      <Link to={"account-preference"} className={`p-8 flex w-full md:!w-1/2 items-center gap-3 bg-dark-10 rounded-2xl  md:mb-6 ${isProfileRoot ? "" : "max-md:hidden"}` }>
        {/*avatar*/}
      ...
      {isLoading ? (
        ...
        <Swiper navigation={true} modules={[Navigation]} spaceBetween={15}  className={`mySwiper cursor-pointer md:!w-1/2 md:!pb-6 max-md:mb-6 ${isProfileRoot ? "" : "max-md:!hidden"}`}>
            ...
            return (
              <>
                ...
                >
                  ...
                  {/* Visit Counter */}
                  <div className="mb-4 p-3 rounded-lg w-5/6" style={{ backgroundColor: 'var(--color-dark-15)' }}>
