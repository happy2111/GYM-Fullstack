...
const ProfileInfo = ({isProfileRoot}) => {
  return (
    <section className={`p-8 flex items-center gap-3 bg-dark-10 rounded-2xl mb-6 ${isProfileRoot ? "" : "max-md:hidden"}` }>
      {/*avatar*/}
      <div className="min-w-16 h-16 font-semibold text-2xl flex items-center justify-center rounded-full overflow-hidden" style={{ backgroundColor: 'var(--color-dark-15)' }}>{authStore.user?.name[0]}</div>
